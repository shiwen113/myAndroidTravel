...
public class Item_Activity extends Activity implements OnClickListener,
	...
	public void isMemberSend() {
		...
		http.send(HttpMethod.POST, urlMember, params,
				new RequestCallBack<String>() {

					@Override
					public void onFailure(HttpException arg0, String arg1) {
						Toast.makeText(getApplication(), "请求失败,请检查您的网络",
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						Type type = new TypeToken<List<TravelMember>>() {
						}.getType();
	...
	public void showData() {
		// 先显示PublishTravel对象
		Type type = new TypeToken<PublishTravel>() {
		}.getType();
		...
		// 在显示旅行队
		if (flag) {
			Type typeList = new TypeToken<List<TravelComment1>>() {
			}.getType();
	...
	public void onClick(View v) {
		...
		case R.id.tv_travel_chakan:
			intentLook = new Intent(Item_Activity.this,
					LookAllPeopleNumberActivity.class);
			sendLookAllPeople();
			break;
	...
	public void sendShouChang() {
		...
		params.addBodyParameter("td", String.valueOf(pt.getTd()));
		http.send(HttpMethod.POST, urlShouChang, params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						if (result != null && !result.equals("null")) {
							...
							if (i == 0) {
								...
								Toast.makeText(getApplication(), "收藏过该旅行队",
										Toast.LENGTH_LONG).show();
							}
						}
					}
	...
	public void sendLookAllPeople() {
		...
		RequestParams params = new RequestParams();
		params.addBodyParameter("td", String.valueOf(pt.getTd()));
		http.send(HttpMethod.POST, urlAll, params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						intentLook.putExtra("time",
								ToolDao.setTimedate1(pt.getStartTime()));
...
//			            myView.setVisibility(View.GONE);
	...
	public void sendInputText() {
		...
		popupWindow = new PopupWindow(myView, getWindowManager()
				.getDefaultDisplay().getWidth(), 300, true);
		...
		http.send(HttpMethod.POST, url, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> arg0) {
				...
				if (string != null && !string.equals("null")) {
					Gson gson = new GsonBuilder().setDateFormat(
							"yyyy-MM-dd hh:mm:ss").create();
					Type type = new TypeToken<List<TravelComment1>>() {
					}.getType();
					if (listContent != null && listContent.size() != 0) {
						listContent.clear();
					}
					listContent.addAll((List<TravelComment1>) (gson.fromJson(
							string, type)));
				}
				// list=gson.fromJson(string,type); list
				// 的引用变了，notifyDataSetChanged会出问题
				// textView.setText(list.get(0).getTd().getLd().getUserName()+"发布的旅行队");

				adapter = new TravelCommentAdapter(Item_Activity.this,
						listContent);
				lv.setAdapter(adapter);
	...
	public void sendJoinTravel(int i) {
		if(m.getLd()!=null){
			if(m.getLd().getLd()!=pt.getLd().getLd()){
		HttpUtils http = new HttpUtils();
		RequestParams params = new RequestParams();
		params.addBodyParameter("td", String.valueOf(pt.getTd()));
		params.addBodyParameter("ld", String.valueOf(m.getLd().getLd()));
		params.addBodyParameter("state", String.valueOf(i));
		http.send(HttpMethod.POST, urlJoin, params,
				new RequestCallBack<String>() {
				...
				Toast.makeText(getApplication(), "您是创建者，不需加入", Toast.LENGTH_SHORT).show();
			}
		}else{
			Toast.makeText(getApplication(), "请先登陆", Toast.LENGTH_SHORT).show();
		}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		if(m.getLd()!=null){
		if (listContent.get(arg2).getLd().getLd() == m.getLd().getLd()) {// 只能弹删除
	...
	public void popPupWindow(View arg1, int arg2) {
		// 局注入器，注入布局给View对象
		View myView1 = getLayoutInflater()
				.inflate(R.layout.deleteorhuifu, null);
		// 通过view 和宽·高，构造PopopWindow
		huifu = (TextView) myView1.findViewById(R.id.tv_huifu);
		delete = (TextView) myView1.findViewById(R.id.tv_delete);
		popupWindow1 = new PopupWindow(myView1, getWindowManager()
				.getDefaultDisplay().getWidth(), 200, true);
	...
	public class onClickHuiFuDelete implements OnClickListener {
		...
		public void onClick(View v) {
			...
			switch (v.getId()) {
			case R.id.tv_huifu:
				// Toast.makeText(getApplication(), "点击成功",
				// Toast.LENGTH_LONG).show();
				myView = getLayoutInflater().inflate(
						R.layout.action_content_edittext, null);
				btnSend = (Button) myView.findViewById(R.id.btn_content_fasong);
				contentEdit = (EditText) myView
						.findViewById(R.id.et_content_data);
				sendInputText();
			...
			case R.id.tv_delete:
				...
				Toast.makeText(getApplication(), "点击成功", Toast.LENGTH_LONG)
						.show();
				break;
			default:
				break;
	...
	public void huiFu(int arg2) {
		...
		if (!contentTo.equals("")) {
			...
			http.send(HttpMethod.POST,
					"http://10.201.1.12:8080/travel/TravelCommentInsert",
	...
	public void delete(int agr2) {
		...
		// params.addBodyParameter("travelcomment1",s1);
		http.send(HttpMethod.POST,
				"http://10.201.1.12:8080/travel/TravelCommentDelete", params,
				new RequestCallBack<String>() {
